...
[SQL语言：DDL、DML、DQL、DCL详解 - 知乎 (zhihu.com)](https://zhuanlan.zhihu.com/p/391552199)
...
从库启动之后，创建一个SQL线程，从relay log里面读取内容，从Exec_Master_Log_Pos位置开始执行
...
例如删除 user 表：drop user;

2. truncate (table)

truncate 是删除表数据，不删除表的结构，速度排第二，但不能与where一起使用

例如删除 user 表：truncate user;
...
删除user表的指定记录 delete from user where user_
...
![img](assets/68747470733a2f2f63732d6e6f7465732d313235363130393739362e636f732e61702d6775616e677a686f752e6d7971636c6f75642e636f6d2f696d6167652d32303139313230373232333430303738372e706e67.png)
...
![img](assets/v2-e60ed5209e491df04c780526c7dc0d36_1440w.webp)
...
![img](assets/v2-87eceeb9a8d6b50d41b841a92d909c1b_1440w.webp)
...
![img](assets/v2-2b8f4d0be2509e2d554fe0cda4a41eeb_1440w.webp)
...
![img](assets/v2-28f1da905b9dacc3f1f3240b9ea5a4b9_1440w.webp)
...
![img](assets/v2-a4ca129185a767ac0dbb48aa213e2b1b_1440w.webp)
...
详细原理看这篇文章：[https://haicoder.net/note/MySQL-interview/MySQL-interview-MySQL-trans-level.html](https://link.zhihu.com/?target=https%3A//haicoder.net/note/MySQL-interview/MySQL-interview-MySQL-trans-level.html)
...
![img](assets/v2-7b7c9f9d0ff11801def918b0729ee689_1440w.webp)
...
当多个用户并发地存取数据时，在[数据库](https://link.zhihu.com/?target=https%3A//cloud.tencent.com/solution/database%3Ffrom%3D10680)中就会产生多个事务同时存取同一数据的情况。若对并发操作不加控制就可能会读取和存储不正确的数据，破坏数据库的一致性。
...
- 行级锁是[MySQL](https://link.zhihu.com/?target=https%3A//cloud.tencent.com/product/cdb%3Ffrom%3D10680)中锁定粒度最细的一种锁，表示只针对当前操作的行进行加锁。行级锁能大大减少数据库操作的冲突。其加锁粒度最小，但加锁的开销也最大。行级锁分为共享锁 和 排他锁。
...
![img](assets/v2-d2074107792d756ffd72672dfaa5ccba_1440w.webp)
...
![img](assets/v2-50efe227f4226272195e7462cdb48268_1440w.webp)
